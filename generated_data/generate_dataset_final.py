...
def parse_annotated(annotated: str):
...
def instantiate_vars(init_block: str, cond_block: str, max_trials: int = 200000):
    init_lines = [ln.strip() for ln in init_block.splitlines() if ln.strip().startswith("-")]
    cond_lines = []
    for ln in cond_block.splitlines():
        ln = ln.strip()
        if ln.startswith("-"):
            cond_lines.append(ln[1:].strip())
    for _ in range(max_trials):
        env = {}
        ok = True
        for line in init_lines:
            assign = line[1:].strip() if line.startswith("-") else line
            ...
            try:
                ...
                ok = False
                break
            parts = [p.strip().replace("$", "") for p in lhs.split(",")]
            if len(parts) == 1:
                env[parts[0]] = value
            else:
                for key, val in zip(parts, value):
                    env[key] = val
        if not ok:
            continue
        try:
...
def build_assistant(answer_text: str):
    sep = "\n####"
    if sep in answer_text:
        reasoning, final = answer_text.split(sep, 1)
...
def generate():
    ...
    for file in template_files:
        ...
        normal.append({
            "template": str(file.relative_to(Path.cwd() if Path.cwd().name=='workspace' else Path('.'))),
            "mode": "normal",
            "system": SYSTEM_PROMPT,
            "user": q_n,
            "assistant": build_assistant(a_n),
        })
        ...
        backtracking.append({
            "template": str(file.relative_to(Path('.'))),
            "mode": "backtracking",
            "system": SYSTEM_PROMPT,
            "user": q_b,
            "assistant": build_assistant(a_b),
        })

    # Balance to 70/30 after generating all samples.
    # Keep all normal samples (one per template), then retain a subset of backtracking samples.
    target_backtracking = int(round(len(normal) * 30 / 70))
    random.shuffle(backtracking)
    final_samples = normal + backtracking[:target_backtracking]
    random.shuffle(final_samples)

    out_file = Path("generated_data/dataset_final.jsonl")
    with out_file.open("w") as f:
        for row in final_samples:
            f.write(json.dumps({k: row[k] for k in ["system", "user", "assistant"]}, ensure_ascii=False) + "\n")

    stats = {
        "templates_total": len(template_files),
        "generated_initial": len(normal) + len(backtracking),
        "normal_count": len(normal),
        "backtracking_count": target_backtracking,
